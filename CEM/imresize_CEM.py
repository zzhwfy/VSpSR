...
import \
    torch  # For using GPU when working with a very large anti-aliasing kernel, e.g. when using extreme
# upscaling/downscaling such as 32x


def imresize(im, scale_factor=None, output_shape=None, kernel=None, align_center=False, return_upscale_kernel=False,
             use_zero_padding=False, antialiasing=True, kernel_shift_flag=False):
    assert kernel is None or any(
        [word in kernel for word in ['cubic', 'blurry_cubic', 'reset_2_default']]) or isinstance(kernel, np.ndarray)
    ...
    assert np.round(scale_factor[0]) == scale_factor[0] or np.round(1 / scale_factor[0]) == 1 / scale_factor[
        0], 'Only supporting integer downsampling or upsampling rates'
    ...
    # Padding the kernel to compenstae for imbalanced padding, in the case of an even scale factor. This increases
    # kernel size by 1 for even scale factors or 0 for odd:
    ...
    if isinstance(kernel, np.ndarray):
        ...
        # assert str(sf_4_kernel) not in imresize.kernels.keys() or np.all(np.equal(kernel,imresize.kernels[str(
        # sf_4_kernel)])),'If using non-default kernel, make sure I always use it.'
        ...
        assert np.all(np.mod(kernel.shape + kernel_post_padding + kernel_pre_padding - 1,
                             sf_4_kernel) == 0), 'Convolution-invalidated size should be an integer multiplication of ' \
                                                 'sf_4_kernel '
    ...
    assert np.all(desired_size == np.round(
        desired_size)), 'Seems like an attempt to downscale with a factor inducing a non-integer image size'
    ...
    def filter2d(input, special_padding_size=None):
        if special_padding_size is not None:
            input = 1 * np.pad(input, pad_width=(
            (special_padding_size[0], special_padding_size[0]), (special_padding_size[1], special_padding_size[1])),
                               mode='edge')
        if antialiasing_kernel.size > 1000:
            print('Using GPU for image resizing (since kernel is of size %dx%d)' % (
            antialiasing_kernel.shape[0], antialiasing_kernel.shape[1]))
            return torch.nn.functional.conv2d(torch.from_numpy(input)
                                              .cuda()
                                              .unsqueeze(0).unsqueeze(0),
                                              torch.from_numpy(
                                                  1 * np.rot90(antialiasing_kernel.astype(input.dtype), 2)).unsqueeze(
                                                  0).unsqueeze(0).cuda(),
                                              padding=(antialiasing_kernel.shape[0] // 2, antialiasing_kernel.shape[
                                                  1] // 2) if special_padding_size is None else 0).squeeze(0).squeeze(
                0).cpu().numpy()
        else:
    ...
    for channel_num in range(im.shape[2]):
        if scale_factor > 1:  # Upscale
            output.append(np.reshape(np.pad(np.expand_dims(np.expand_dims(im[:, :, channel_num], 2), 1), (
            (0, 0), (pre_stride[0], post_stride[0]), (0, 0), (pre_stride[1], post_stride[1])),
...
def Center_Mass(kernel, ds_factor):
    ...
    (Round_2_Int(pre_y_pad), Round_2_Int(post_y_pad)), (Round_2_Int(pre_x_pad), Round_2_Int(post_x_pad))),
                    mode='constant')
    assert kernel.shape[0] == kernel.shape[1], 'I caused the kernel to stop being a square...'
    margins_2_remove = np.argwhere(Return_Filter_Energy_Distribution(kernel) < 0.99)[0][0] * np.ones([2]).astype(
        np.int32)
    pre_post_index = 0
...
def Return_Filter_Energy_Distribution(filter):
    sqrt_energy = [np.sqrt(np.sum(filter ** 2))] + [
        np.sqrt(np.sum(filter[frame_num:-frame_num, frame_num:-frame_num] ** 2)) for frame_num in
        range(1, int(np.ceil(filter.shape[0] / 2)))]
